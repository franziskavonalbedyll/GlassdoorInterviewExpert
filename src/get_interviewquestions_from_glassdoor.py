...
def extract_interview_questions_from_html(html):
    ...
    interview_questions_elements = soup.find_all(attrs={"data-test": lambda value: value and value.startswith('Interview') and value.endswith('QuestionsContainer')})

    # Removing "Interview Questions" and "Answer Question" from the strings
    cleaned_interview_questions = [' '.join(element.stripped_strings).replace("Interview Questions", "").replace("Answer Question", "").strip() for element in interview_questions_elements]
...
def collect_questions(base_url, stop_after = float('inf')):
    ...
    while True:
        ...
        html = get_html(url)
        cleaned_interview_questions = extract_interview_questions_from_html(html)
